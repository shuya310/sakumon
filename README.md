...
## 支援（フェーズ2）— `docs/spec/sakumon_spec_v3.md`
...
| 役割回答の正答率 | 弱段階ターン1で、除数が場面で何を表すかを正しく答えた割合（role_answer、訂正前の値） | P2 | 除数の役割の自覚 |
...
| 役割回答の正答率 | `input_type='role'` の行の `role_answer`。**期待される役割は列として持たない**ので、その行の直前の成立行（同セッション・`log_id` が小さい最新の `valid=1`）の `structure` / `unknown` から `ai_dialogue.expected_divisor_role` で導く（tobun→people、hougan→per_one、bai の ratio/rate→base。bai の base（除数が倍率）は期待役割を確定できないので分母から除く）。訂正前の値は各役割の問いに対する**1回目**の role 行（`role_corrected=1` の行、またはその直前に訂正が無い行）。`role_corrected=1` は「期待役割が確定でき、答えが people/per_one/base のどれかで、不一致」を意味するので、確定できる行では 正答 ⇔ `role_corrected=0` かつ `role_answer ∈ {people, per_one, base}`。`dont_know` / `unknown` は誤答側に数える（分母に含める） |
