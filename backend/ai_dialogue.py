...
気づかせ、停滞や「わからない」が続くほどhint1→hint2→hint3と少しずつ具体的に
していく（hint1:問いかけのみ／hint2:今できている構造の図で比べさせる／
hint3:まだ作っていない構造の見方を図とともに示す）。段階は main.py が
...
SUPPORT_LEVEL_JP = {
    ...
    "hint2": "hint2（くり返しが続く、または『同じ話じゃないの？』『わからない』と言われた："
             "ここで初めて図を出してよい。ただし図は“いま作った（できている）構造”を示し、"
             "何をさがしているかをふり返らせる。次の構造の名前や求め方はまだ出さない）",
    "hint3": "hint3（hint2でも変化がない／混乱がくり返される：まだ作っていない構造への"
             "見方（さがし方）を、図とともにはじめて具体的に示してよい）",
...
- hint2（くり返しが続く／「同じ話じゃないの？」「わからない」等）：ここで初めて
  図を出してよい。ただし図は“いま作った（すでにできている）構造”を示し、
  「今のお話は何をさがしているか」をふり返らせることが目的。
  題材（りんご→えんぴつ 等）ではなく構造（さがしているもの）で比べれば
  同じだと気づかせる。次に何をさがせばよいかは問いかけで終え、まだ名指ししない。
- hint3（hint2でも変化がない／混乱がくり返される）：ここではじめて、
  まだ作っていない構造への「見方（さがし方）」を具体的に示してよい。
  題材を変えることではなく、“何をさがすか”の違いに目を向けさせる。
...
通常hint2以上になっている。題材ではなく「何をさがしているか（さがし方）」で見れば
同じだと気づかせることを、いま作った構造の図とともに示す。
...
- hint1：null（図はまだ出さない）。
- hint2："いま作った（すでにできている）構造"を入れる。まだ作っていない構造は入れない。
- hint3：まだ作っていない構造をひとつ入れる。
- goal："all"。
        ...
        try:
            ...
            if message:
                ...
                if figure not in ("tobun", "hougan", "bai", "all"):
                    figure = None
                # form/talk/hint1では図を出さない。プロンプト任せにせずコード側でも強制する。
                if support_level in ("form", "talk", "hint1"):
                    figure = None
