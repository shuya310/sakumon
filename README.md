...
valid / structure / display_type / message / figure を返す
        ↓
フロントエンドがチャット形式でフィードバック表示（figure が入っていればテープ図SVGも表示）
```
...
`ai_dialogue.py` が返す `figure`（構造名: `tobun`/`hougan`/`bai`/`all`/`null`）をもとに、
`frontend/index.js` の `figureSvg()` が構造ごとのテープ図SVGを**決定論的に**描画する（**実装済み**）。
`figure:"all"` のときは3構造のミニ図を並べて表示する（ゴール到達時）。再開時の会話復元
（`renderConversation()`）でも過去に出した図を再描画する。

なお「新しい構造ができたとき」の figure について、プロンプト上は discover 段階で
「**できたばかりの構造**」を出す指示になっているが、実測ではAIが「**次に作らせたい構造**」を
返す傾向がある（例: 等分除ができたとき `figure:"hougan"`）。バグではなくプロンプト解釈の差。
描画が実装済みになったことで**画面に直接影響する**ため、⚠️ **優先して確認・修正すべき未解決事項**。
...
| 管理者画面 | 全ユーザーのログ閲覧（作問/対話種別・つまづき分類・出した図を表示）・CSV出力・削除 |
