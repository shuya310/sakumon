...
help / talk で 0→1 に上がったターンは、talk の文言（強度0で生成）の後ろに弱の文言（現在地の対比＋「4を どんな ふうに 使った お話に する？」）を連結し、宣言を待つ。help で 1→2・2→3 に上がったターンも同様に、talk の文言の後ろに中・強の文言（目標＝児童の宣言かシステム指定。題材は最新の成立作問）を連結する（9/18：連結が無いと上部の「つぎは」だけが変わり、行き先の指示が次の成立作問まで出なかった）。いずれも成立作問が無ければ連結しない。
