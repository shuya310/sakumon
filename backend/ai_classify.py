...
# ===== 予告（自由記述）の分類 =====
# 仕様 v3.2：弱の問い「じゃあ 次は、何を 求める お話に する？」への児童の答えを tobun / hougan / bai / unknown に分ける。
# 判断基準は「何を求める（またはわる数をどう使う）と書いているか」だけ。
...
「じゃあ 次は、何を 求める お話に する？」と聞かれ、それに答えました。
その答えを次の4つに分類します。分類結果のJSONだけを返してください。JSON以外の文字は一切出力しないでください。

- "tobun"：1人分・1つ分の大きさを求めると書いている（「何こか」「1人分」「1人何こ」「1つ分」「1人分は何本」など、
  1人（1つ）あたりの数を求める）、または {divisor} を「分ける相手の数」（{divisor}人で分ける・{divisor}つの班・{divisor}チーム・{divisor}ふくろに分ける）として使う
- "hougan"：何人に配れるか・何人に分けられるか・いくつ分・まとまりの数を求めると書いている（「何人か」「何人に分けられるか」「何ふくろか」「何チームか」など）、
  または {divisor} を「1人分・1つ分の数」（{divisor}こずつ・1人に{divisor}こ・{divisor}まいずつ配る）として使う
- "bai"：何倍かを求めると書いている（「何倍か」「何倍」「くらべる」）、または {divisor} を「くらべる相手の量」（{divisor}こと くらべる・{divisor}この何倍）として使う
- "unknown"：題材だけ（「りんごの問題」「クラスの話」）・「わからない」・質問で返している（「どういうこと？」）・無関係・判断できない

注意：
- 「何こか」「何本か」「何こずつか」「1人何こか」のように 1人分（1つ分）の量を求める言葉は "tobun"。
  「何人か」「何ふくろか」「何チームか」のようにまとまりの数を求める言葉は "hougan"。
- 数 {divisor} の使い方で答えているときは、{divisor} の役割で分ける：「{divisor}こずつ」「1人に{divisor}こ」は 1人分が {divisor} なので "hougan"
  （求めるのは何人か）。「{divisor}人で」「{divisor}つの班」は分ける相手が {divisor} なので "tobun"（求めるのは1人分）。
