...
| 2 | 支援：フィードバックを受けながら作問（新セッション。フェーズ1は引き継がない） | 式A | ＋ AIの声かけ、水準1以降「作ったお話」、水準3以降 信号機（ラベルなしの3灯） |
...
| 1 | `level1` | 産出一覧（問題文のみ） |
| 2 | `level2` | 一覧＋「聞いていることは同じ？ちがう？」＋2択ボタン |
| 3 | `level3` | 求める量の明示（1つ分／いくつ分／何倍）＋信号機 |
| 4 | `level4` | 場面想起の足場かけ（素材のみ。数量関係は渡さない） |
| ─ | `discover` | 新構造（称賛のみ） |
| ─ | `goal` | 3構造そろった |
| ─ | `talk` | 作問以外の入力 |
| ─ | `none` | フェーズ1・3（表示なし） |

- 遷移の単位は作問の提出。既出構造の反復で1段階ずつ上がり（上限4）、新構造で1にリセット。
- 水準2で「ちがう」と答えた場合だけ、提出を待たず水準3へ。
- 水準1〜3・goal は固定文。`form` / `discover` / `level4` / `talk` は LLM 生成＋コード側ガード（構造名・「ずつ」・被除数と除数の同時出現などを含むと定型文へ差し替え）。
